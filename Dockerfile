...
RUN --mount=type=cache,id=cache-npm,target=/root/.npm npm ci --ignore-scripts --omit-dev

# Copy source code
COPY . .

# Build the package
RUN --mount=type=cache,id=cache-npm,target=/root/.npm npm run build

# Install package globally
RUN --mount=type=cache,id=cache-npm,target=/root/.npm npm link
